...
export function spawnLightningBolt(clickX, clickY, lightningCount, delay) {
  ...
  let startX, startY;
  if (startSide < 0.6) { startX = clickX + (Math.random() - 0.5) * canvas.width * 0.4; startY = -10; }
  else if (startSide < 0.8) { startX = -10; startY = Math.random() * canvas.height * 0.4; }
  else { startX = canvas.width + 10; startY = Math.random() * canvas.height * 0.4; }
  ...
  for (let b = 0; b < branchCount; b++) {
    ...
    const branchEnd = { x: branchStart.x + Math.cos(branchAngle) * branchLen, y: branchStart.y + Math.sin(branchAngle) * branchLen };
    branches.push(generateBoltPath(branchStart.x, branchStart.y, branchEnd.x, branchEnd.y, 4, jitter * 0.5));
  }

  lightningBolts.push({
    mainPath: mainPath, branches: branches,
    life: 1.0, decay: 0.035, intensity: intensity,
    delay: delay || 0, glowWidth: 3 + lightningCount * 0.5,
  });

  halos.push({ type: 'glow', x: clickX, y: clickY, maxRadius: 50 * intensity, opacity: 0.7 * intensity, life: 1.0, decay: 0.03, delay: delay || 0 });
}
...
export function spawnFissureCrack(clickX, clickY, count, delay) {
  ...
  for (let i = 1; i < segments; i++) {
    const t = i / segments;
    points.push({ x: clickX + dx * t + (Math.random() - 0.5) * jitter, y: clickY + dy * t + (Math.random() - 0.5) * jitter * 0.5 });
  }
  points.push({ x: endX, y: endY });

  halos.push({ type: 'void-crack', x: clickX, y: clickY, maxRadius: crackLen, opacity: intensity * 0.9, life: 1.0, decay: 0.02, delay: delay || 0, points: points, crackWidth: 2 + count * 0.4 });
  ...
  for (let b = 0; b < branchCount; b++) {
    ...
    const branchEnd = { x: branchPt.x + Math.cos(branchAngle) * branchLen, y: branchPt.y + Math.sin(branchAngle) * branchLen };
    const branchPoints = [branchPt];
    for (let s = 1; s < 3; s++) {
      const bt = s / 3;
      branchPoints.push({ x: branchPt.x + (branchEnd.x - branchPt.x) * bt + (Math.random() - 0.5) * jitter * 0.3, y: branchPt.y + (branchEnd.y - branchPt.y) * bt + (Math.random() - 0.5) * jitter * 0.3 });
    }
    branchPoints.push(branchEnd);
    halos.push({ type: 'void-crack', x: branchPt.x, y: branchPt.y, maxRadius: branchLen, opacity: intensity * 0.5, life: 1.0, decay: 0.025, delay: (delay || 0) + 2, points: branchPoints, crackWidth: 1 + count * 0.2 });
  }

  halos.push({ type: 'void-collapse', x: clickX, y: clickY, maxRadius: 40 * intensity, opacity: 0.6 * intensity, life: 1.0, decay: 0.025, delay: delay || 0 });
}
...
export function drawLightningBolts() {
  for (const bolt of lightningBolts) {
    ...
    for (const branch of bolt.branches) { drawBoltPath(branch, alpha * 0.6, bolt.glowWidth * 0.5); }
    ctx.restore();
  }
}
